...
::: notes
FROM CLAUDE: These diagrams show how querychat works under the hood — the LLM receives the user's question, writes a SQL query, and the database executes it. The model never sees your raw data.
:::
...
::: notes
FROM CLAUDE: For those who want to go further — the querychat docs show how to embed it in a full Shiny dashboard with plots, value boxes, and more. That's what you'll do in the next section with Posit Assistant.
:::
...
::: notes
FROM CLAUDE: shinychat is the lower-level building block that querychat is built on. If you want to build your own custom chat interface in Shiny — not necessarily tied to data querying — shinychat gives you the UI components.
:::
